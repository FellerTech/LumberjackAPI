...
/**
* @file lumberjack_api_defs.hpp
* @brief Lumberjack C++ API, an extensible wrapped API that is exposed by the DLLs.
*
...
* To make use of this file, you should include lumberjack_api.hpp in a client source file;
...
// Everything defined here lives in the lumberjack namespace.
namespace lumberjack {
  ...
  /// lumberjack_StreamProperties C structure and provides a C++ interface for it.
  ...
  public:
    ...
    /// call that does not itself return an lumberjack_Status to ensure that the operation
    /// completed.
    /// @return lumberjack_Status returned by the most-recent operation on the wrapped
    ///         class, or other errors in case the StreamProperties itself is broken.
    lumberjack_Status GetStatus();
    ...
    /// @return Returns lumberjack_STATUS_OKAY on success and a specific code on failure.
    ///         GetStatus() does not need to be called after this method because it
    ///         is returned here.
    lumberjack_Status Rate(double rate);
  ...
  protected:
    ...
    ::std::shared_ptr<lumberjack_StreamProperties_> GetRawProperties() const;
    /// @cond INTERNAL
  ...
  /// C++ analog of the lumberjack_APIDataBlobSourceInfo C structure.  It is a data-only
  ...
  /// system.
  ///
  /// This is returned by lumberjack::API::GetAvailableDataBlobSources().
  ...
  /// lumberjack_Message C structure and provides a C++ interface for it.
  /// The GetStatus() method should be called after each method (including
  /// the constructor) to make sure that the operation was a success.
  ...
  public:
	  ...
	  Message(::std::string value, struct timeval time,
		  lumberjack_MessageLevel level);

	  /// @brief Creates a Message from an lumberjack_Message.
	  /// @param [in] Message The lumberjack_Message to refer to when calling this
	  ///             object's methods.  It is copied on construction and
	  ///             destroyed in ~Message().
	  Message(lumberjack_Message Message);
    ...
    ///         GetStatus() does not need to be called after this method because it
    ///         is returned here.
    lumberjack_Status Value(::std::string value);
    ...
    ///         GetStatus() does not need to be called after this method because it
    ///         is returned here.
    lumberjack_Status TimeStamp(struct timeval value);

	  /// @brief Get the level of the message.
	  lumberjack_MessageLevel Level() const;
    ...
    ///         GetStatus() does not need to be called after this method because it
    ///         is returned here.
    lumberjack_Status Level(lumberjack_MessageLevel value);

	  /// @brief Accessor for the passed-in information during construction, not used by client code.
	  lumberjack_Message const RawMessage() const;
  ...
  /// lumberjack_API C structure and provides a C++ interface for it.
  /// The GetStatus() method should be called after each method (including
  /// the constructor) to make sure that the operation was a success.
  ...
  public:
    ...
    lumberjack_VERSION GetVersion() const;
    ...
    lumberjack_Status SetVerbosity(uint16_t verbosity);
    ...
    /// @return lumberjack_STATUS_OKAY on success, a specific error code on failure.
    ///         GetStatus() should not be called after this method, since it is returned here.
    lumberjack_Status SetLogMessageCallback(LogMessageCallback callback,
    ...
    /// @return lumberjack_STATUS_OKAY on success, a specific error code on failure.
    ///         GetStatus() should not be called after this method, since it is returned here.
    lumberjack_Status SetLogMessageStreamingState(bool running = true);
    ...
    /// @param level Minimum level to be returned, defaults to lumberjack_MESSAGE_MINIMUM_INFO.
    /// @return lumberjack_STATUS_OKAY on success, a specific error code on failure.
    ///         GetStatus() should not be called after this method, since it is returned here.
    lumberjack_Status SetLogMessageMinimumLevel(lumberjack_MessageLevel level);
  ...
  protected:
    /// @brief Protected method for use by the API implementation.
    lumberjack_API GetRawAPI() const;
  ...
  /// lumberjack_DataBlob C structure and provides a C++ interface for it.
  /// The GetStatus() method should be called after each method (including
  /// the constructor) to make sure that the operation was a success.
  ///
  ...
  /// object does not release the underlying data.
    ...
    public:
      ...
      /// @brief Used internally to construct based on an lumberjack_DataBlob.
      ///
      /// Makes a copy of the lumberjack_DataBlob and destroys the copy during the
      /// deconstruction.  Does not call ReleaseData() upon destruction -- the
      /// client is responsible for doing this.
      /// @param [in] blob lumberjack_DataBlob that is copied to construct this class.
      DataBlob(lumberjack_DataBlob blob);
      ...
      lumberjack_DataBlob const RawDataBlob() const;
    ...
    /// lumberjack_DataBlobSource C structure and provides a C++ interface for it.
    /// The GetStatus() method should be called after each method (including
    /// the constructor) to make sure that the operation was a success.
    ///
    ...
    public:
      ...
      ///        lumberjack::API::GetAvailableDataBlobSources(); defaults to any available DataBlobSource.
      DataBlobSource(
        API &api,
      ...
      /// @return lumberjack_STATUS_OKAY on success, a specific error code on failure.
      ///         GetStatus() should not be called after this method, since it is returned here.
      lumberjack_Status SetStreamCallback(StreamCallback callback, void *userData = nullptr);
      ...
      /// @return lumberjack_STATUS_OKAY on success, a specific error code on failure.
      ///         GetStatus() should not be called after this method, since it is returned here.
      lumberjack_Status SetStreamingState(bool running = true);
